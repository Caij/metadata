...
public class Mp4InfoReader {
    ...
    public GenericAudioHeader read(BuffInputStream raf) throws CannotReadException, IOException {
        ...
        if (trak.getStco() != null && trak.getStco().getChunkOffsets().length > 0) {
            long[] offsets = trak.getStco().getChunkOffsets();
            info.setAudioDataStartPosition(offsets[0]);
            info.setAudioDataEndPosition(raf.size());
            info.setAudioDataLength(raf.size() - offsets[0]);
        }

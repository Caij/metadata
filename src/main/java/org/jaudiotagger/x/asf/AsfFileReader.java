...
public class AsfFileReader extends AudioFileReader {
    ...
    protected GenericAudioHeader getEncodingInfo(final SlideBufferFileChannel raf) throws CannotReadException, IOException {
        ...
        try {
            final AsfHeader header = ID3V2TagUtil.readInfoHeader(raf);
            if (header == null) {
    ...
    protected AsfTag getTag(final SlideBufferFileChannel raf) throws CannotReadException, IOException {
        ...
        AsfTag tag;
        try {
            final AsfHeader header = ID3V2TagUtil.readTagHeader(raf);
            if (header == null) {
                throw new CannotReadException("Some values must have been " + "incorrect for interpretation as asf with wma content.");
            }
    ...
    public XAudioFile read(final ChannelCompat f) throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException {
        ...
        try {
            ...
            if (header.getFileHeader() == null) {
                throw new CannotReadException(ErrorMessage.ASF_FILE_HEADER_MISSING.getMsg(""));
            }

            // Just log a warning because file seems to play okay
            if (header.getFileHeader().getFileSize().longValue() != f.size()) {

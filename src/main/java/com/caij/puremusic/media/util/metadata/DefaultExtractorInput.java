...
/** An {@link androidx.media3.extractor.ExtractorInput} that wraps a {@link DataReader}. */
@UnstableApi
public final class DefaultExtractorInput implements ExtractorInput {
  ...
  public boolean readFully(byte[] target, int offset, int length, boolean allowEndOfInput)
    ...
    while (bytesRead < length && bytesRead != Util.RESULT_END_OF_INPUT) {
      bytesRead = readFromUpstream(target, offset, length, bytesRead, allowEndOfInput);
    }
    commitBytesRead(bytesRead);
    return bytesRead != Util.RESULT_END_OF_INPUT;
  }
  ...
  public boolean skipFully(int length, boolean allowEndOfInput) throws IOException {
    int bytesSkipped = skipFromPeekBuffer(length);
    while (bytesSkipped < length && bytesSkipped != Util.RESULT_END_OF_INPUT) {
    ...
    commitBytesRead(bytesSkipped);
    return bytesSkipped != Util.RESULT_END_OF_INPUT;
  }
  ...
  public int peek(byte[] target, int offset, int length) throws IOException {
    ...
    if (peekBufferRemainingBytes == 0) {
      ...
      if (bytesPeeked == Util.RESULT_END_OF_INPUT) {
        return Util.RESULT_END_OF_INPUT;
      }
  ...
  private int readFromUpstream(
    ...
    if (bytesRead == Util.RESULT_END_OF_INPUT) {

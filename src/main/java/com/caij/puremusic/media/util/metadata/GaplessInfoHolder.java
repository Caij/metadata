...
import com.caij.puremusic.media.util.metadata.data.id3.CommentFrame;
import com.caij.puremusic.media.util.metadata.data.id3.InternalFrame;
...
public final class GaplessInfoHolder {
  ...
  private boolean setFromComment(String data) {
    ...
    if (matcher.find()) {
      try {
        int encoderDelay = Integer.parseInt(Util.castNonNull(matcher.group(1)), 16);
        int encoderPadding = Integer.parseInt(Util.castNonNull(matcher.group(2)), 16);
